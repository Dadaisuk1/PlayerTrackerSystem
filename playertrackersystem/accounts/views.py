...
def logout_view(request):
    if 'player_id' in request.session:
        del request.session['player_id']  # Remove player ID from session
...
def register_player(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()  # Save the new player, password will be hashed
...
def update_player(request, playerID):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('player_list')  # Redirect to player list after saving
    else:
...
def delete_player(request, playerID):
    ...
    if request.method == 'POST':
        ...
        return redirect('player_list')  # Redirect to player list after deletion
